...
def main():
    ...
    model.fc = nn.Sequential(nn.Linear(2048, 1024),
                             nn.GELU(), nn.Linear(1024, config.n_vertices*5))
    ...
    for epoch in range(config.n_epochs):
        ...
        for index, img in enumerate(tqdm(dataloader)):
            ...
            average_pairwise_dist = torch.cdist(
                coordinates, coordinates).min(dim=-1)[0].mean()
            ...
            loss = 2.5*sigmoid(l1_loss) - \
                sigmoid(average_pairwise_dist)  # 2 kinda worked
            ...
            ), "Pairwise dist loss": average_pairwise_dist, "lr": optimizer.param_groups[0]['lr']})
